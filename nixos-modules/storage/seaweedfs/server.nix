...
}:
{
  config = lib.mkIf config.skyflake.storage.seaweedfs.enable {
    ...
    systemd.services = lib.mkMerge [
        ...
        seaweedfs-master = {
          ...
          serviceConfig =
            let
              ...
              peers = "${lib.concatMapStrings (x: x + ":9333,") (
                builtins.catAttrs "address" (builtins.attrValues config.skyflake.nodes)
              )}";
            in
            {
              Type = "simple";
      ...
      (lib.mapAttrs' (
        name: value:
        lib.nameValuePair ("seaweedfs-mount" + (lib.replaceStrings [ "/" ] [ "-" ] name)) {
          ...
          serviceConfig = {
            ...
            ExecStart = ''${pkgs.seaweedfs}/bin/weed mount -nonempty -filer=localhost:8888 ${
              lib.optionalString (
                !builtins.isNull value.replication
              ) "-replication=${builtins.toString value.replication}"
            } -cacheCapacityMB=${builtins.toString value.cacheCapacity} -chunkSizeLimitMB=${builtins.toString value.chunkSizeLimit} -dirAutoCreate -dir=${builtins.toString name} -filer.path=${value.mountSource}'';
